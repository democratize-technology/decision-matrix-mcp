...
from decision_matrix_mcp.models import Criterion, DecisionSession, ModelBackend, Score
...
class TestPerformanceOptimizations:
        ...
        for option_name, option in self.session.options.items():
            for criterion_name in self.session.criteria:
                score = Score(
                    ...
                    score=7.5
                    + (hash(f"{option_name}_{criterion_name}") % 30) / 10,  # 7.5-10.0 range
    ...
    def test_small_dataset_performance(self):
        ...
        assert (
            execution_time < 0.1
        ), f"Small dataset should complete in <0.1s, took {execution_time:.3f}s"
    ...
    def test_medium_dataset_performance(self):
        ...
        assert (
            execution_time < 0.5
        ), f"Medium dataset should complete in <0.5s, took {execution_time:.3f}s"
        assert (
            memory_usage < 5 * 1024 * 1024
        ), f"Memory usage should be <5MB, used {memory_usage} bytes"
        assert "error" not in result
    ...
    def test_large_dataset_performance(self):
        ...
        assert (
            execution_time < 1.0
        ), f"Large dataset should complete in <1.0s, took {execution_time:.3f}s"
        assert (
            memory_usage < 10 * 1024 * 1024
        ), f"Memory usage should be <10MB, used {memory_usage} bytes"
        assert "error" not in result
    ...
    def test_caching_effectiveness(self):
        ...
        # First call - no cache
        execution_time_1, _, result_1 = self.measure_performance(self.session.get_decision_matrix)

        # Second call - should use cache
        execution_time_2, _, result_2 = self.measure_performance(self.session.get_decision_matrix)
    ...
    def test_weighted_total_optimization(self):
        ...
        execution_time, memory_usage, result = self.measure_performance(
            option.get_weighted_total,
            self.session.criteria,
        )

        assert (
            execution_time < 0.01
        ), f"Weighted total should complete in <0.01s, took {execution_time:.4f}s"
    ...
    def test_score_breakdown_optimization(self):
        ...
        execution_time, memory_usage, result = self.measure_performance(
            option.get_score_breakdown,
            self.session.criteria,
        )

        assert (
            execution_time < 0.01
        ), f"Score breakdown should complete in <0.01s, took {execution_time:.4f}s"
    ...
    def test_memory_efficiency_slots(self):
        ...
        assert (
            bytes_per_score < 1000
        ), f"Each Score should use <1KB, using {bytes_per_score:.0f} bytes"
    ...
    def test_xl_dataset_benchmark(self):
        ...
        assert (
            execution_time < 5.0
        ), f"XL dataset should complete in <5.0s, took {execution_time:.3f}s"
        assert (
            memory_usage < 50 * 1024 * 1024
        ), f"Memory usage should be <50MB, used {memory_usage} bytes"
        assert "error" not in result
