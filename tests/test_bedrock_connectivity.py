...
class TestBedrockConnectivityOrchestrator:
    ...
    async def test_bedrock_connection_success(self, orchestrator):
        ...
        with patch('decision_matrix_mcp.orchestrator.BOTO3_AVAILABLE', True), \
    ...
    def test_get_bedrock_error_suggestion(self, orchestrator):
        ...
        # Region error
        suggestion = orchestrator._get_bedrock_error_suggestion("ValidationException", "region not available")
        assert "Try us-east-1 or us-west-2" in suggestion

        # Credentials error
        suggestion = orchestrator._get_bedrock_error_suggestion("UnauthorizedOperation", "credentials not found")
        assert "Configure AWS credentials" in suggestion

        # Throttling error
        suggestion = orchestrator._get_bedrock_error_suggestion("ThrottlingException", "throttling limit exceeded")
        assert "Request rate limit exceeded" in suggestion

        # Unknown error
        suggestion = orchestrator._get_bedrock_error_suggestion("UnknownError", "something went wrong")
