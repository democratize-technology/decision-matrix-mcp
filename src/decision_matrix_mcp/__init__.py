...
from .session_manager import session_manager, SessionValidator
...
class StartDecisionAnalysisRequest(BaseModel):
    ...
    initial_criteria: Optional[List[Dict[str, Any]]] = Field(
        default=None,
...
async def evaluate_options(request: EvaluateOptionsRequest) -> Dict[str, Any]:
    ...
    try:
        logger.info(f"Starting evaluation: {len(session.options)} options × {len(session.criteria)} criteria")
