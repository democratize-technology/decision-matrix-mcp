...
class SessionManager:
    ...
    def __init__(self, max_sessions: int = 50, session_ttl_hours: int = 24, cleanup_interval_minutes: int = 30):
    ...
    def create_session(self, topic: str, initial_options: Optional[list] = None) -> DecisionSession:
        ...
        if len(self.sessions) >= self.max_sessions:
            ...
            if len(self.sessions) >= self.max_sessions:
                raise Exception(f"Session limit reached ({self.max_sessions}). Please try again later.")
    ...
    def _cleanup_if_needed(self):
        ...
        if datetime.now(UTC) - self.last_cleanup > self.cleanup_interval:
            self._cleanup_expired_sessions()
    ...
    def _is_session_expired(self, session: DecisionSession) -> bool:
        ...
        return datetime.now(UTC) - session.created_at > self.session_ttl
